...
def initDB():
    ...
    cur = con.cursor()
    cur.execute("CREATE TABLE IF NOT EXISTS routes (id INTEGER PRIMARY KEY AUTOINCREMENT, rule_set, selector, selector_key, selector_value, destination, multipath, gateway, interface)")
    con.commit()
...
def create_route(rule_set, destination, selector=None, selector_key=None, selector_value=None, multipath=None, gateway=None, interface=None):
    str_multipath = json.dumps({"multipath":multipath})
    str_selector_value = json.dumps({"selector_value":selector_value})
    con = sqlite3.connect(dbfile)
    cur = con.cursor()
    cur.execute(f"INSERT INTO routes(rule_set, selector, selector_key, selector_value, destination, multipath, gateway, interface) VALUES ('{rule_set}', '{selector}', '{selector_key}', '{str_selector_value}', '{destination}', '{str_multipath}', '{gateway}', '{interface}')")
    id = cur.lastrowid
    con.commit()
    return id

def update_route(id, rule_set, destination, selector=None, selector_key=None, selector_value=None, multipath=None, gateway=None, interface=None):
    str_multipath = json.dumps({"multipath":multipath})
    str_selector_value = json.dumps({"selector_value":selector_value})
    con = sqlite3.connect(dbfile)
    cur = con.cursor()
    cur.execute(f"UPDATE routes SET rule_set='{rule_set}', selector='{selector}', selector_key='{selector_key}', selector_value='{str_selector_value}', destination='{destination}', multipath='{str_multipath}', gateway='{gateway}', interface='{interface}' WHERE id={id}")
    con.commit()
    return id
...
def add(current_user):
    ...
    if result:
        id = update_route(id=result[0],rule_set=rule_set,destination=destination,selector=selector,selector_key=selector_key,selector_value=selector_value,multipath=multipath,gateway=gateway,interface=interface)
        return {"message":f"updated with id:{id}"}
    else:
        id = create_route(rule_set=rule_set,destination=destination,selector=selector,selector_key=selector_key,selector_value=selector_value,multipath=multipath,gateway=gateway,interface=interface)
